import re
import json
from typing import Dict, Any, Tuple
from src.core.observability import Logger

logger = Logger("intent_classifier")

class QueryIntentClassifier:
    def __init__(self, call_llm_fn):
        self.call_llm = call_llm_fn
        self.valid_intents = [
            "factual question", "summarization", "chapter summary", "key concepts",
            "important quote", "important line", "definition", "comparison",
            "reasoning", "follow-up", "document overview", "explain section",
            "analytical question"
        ]

    def classify_locally(self, query: str) -> str:
        """Rule-based local fallback classification."""
        query_lower = query.lower()
        
        # Check Quote extraction intents
        if any(w in query_lower for w in ["quote", "line", "sentence", "takeaway", "takeaways", "memorable"]):
            return "important quote"
            
        # Check summarization intents
        if any(w in query_lower for w in ["summarize", "summary", "overview", "brief", "concise summary", "what is this document about"]):
            if "chapter" in query_lower or "section" in query_lower:
                return "chapter summary"
            return "summarization"
            
        # Check comparisons
        if any(w in query_lower for w in ["compare", "difference", "versus", "vs"]):
            return "comparison"
            
        # Check definitions
        if any(w in query_lower for w in ["define", "definition", "what is", "meaning of"]):
            return "definition"
            
        # Check reasoning/analytics
        if any(w in query_lower for w in ["why", "how", "reason", "explain", "analytical"]):
            return "reasoning"
            
        # Default
        return "factual question"

    def classify(self, query: str, history_str: str = "") -> Dict[str, Any]:
        """Classifies the query intent using LLM with local rule-based fallback."""
        system_prompt = (
            "You are an expert intent classifier. Categorize user queries based on the target intent. "
            "Respond strictly in JSON format within <answer> </answer> tags."
        )
        
        prompt = f"""Classify the user's search query intent.
        
Recent conversation history (for follow-up intent classification):
{history_str}

User's current query: "{query}"

Available intents:
{self.valid_intents}

Output JSON format:
<answer>
{{
    "intent": "one of the available intents",
    "summary_mode": "short" | "detailed" | "bullet" | "chapter-wise" (only if intent is summarization or chapter summary),
    "explanation": "concise explanation of intent selection"
}}
</answer>
"""
        try:
            provider, response_text = self.call_llm(prompt, system_prompt)
            if "Demo Fallback" in provider:
                # LLM offline, fallback
                intent = self.classify_locally(query)
                return {"intent": intent, "summary_mode": "short", "explanation": "Fallback local classification"}
                
            ans_match = re.search(r'<answer>(.*?)(</answer>|$)', response_text, re.DOTALL)
            if ans_match:
                result = json.loads(ans_match.group(1).strip())
            else:
                json_match = re.search(r'\{.*\}', response_text, re.DOTALL)
                if json_match:
                    result = json.loads(json_match.group(0))
                else:
                    raise ValueError("No JSON block found")
                    
            intent = result.get("intent", "").lower().strip()
            if intent not in self.valid_intents:
                intent = self.classify_locally(query)
                
            summary_mode = result.get("summary_mode", "short")
            return {
                "intent": intent,
                "summary_mode": summary_mode,
                "explanation": result.get("explanation", "LLM classified")
            }
        except Exception as e:
            logger.warn("Query classification failed, running local rules", error=str(e))
            intent = self.classify_locally(query)
            return {"intent": intent, "summary_mode": "short", "explanation": "Fallback local classification"}
